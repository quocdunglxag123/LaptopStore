	...
	<form action="NhaCungCapDB" method="post">
		...
		<section class="body_product">
			<div class="container">
				...
				<a href="InsertNhaCungCapDB">Add Nhà Cung Cấp</a>
				<div class="show_product">
					<table border=1>
						...
						<c:forEach items="${listNhaCungCap}" var="ncc" begin="${start }"
								end="${start+9}">
							<tr>
							<%-- 	<td>${ncc.maNCC}</td> --%>
								<td>${ncc.tenNCC}</td>
								<td>${ncc.diaChi}</td>
								<td>${ncc.dienThoai}</td>
								<td><a href="LoadEditNhaCungCapDB?maNCC=${ncc.maNCC}">Edit</a><a
									href="DeleteNhaCungCap?id=${ncc.maNCC}">Delete</a></td>
							</tr>
						</c:forEach>
					</table>
					...
					<c:if test="${start>0}">
						<a class="nextprevious" href="NhaCungCapDB?start=${start-10}"
					...
					<a class="nextprevious" href="NhaCungCapDB?start=0">1</a>
					<c:if test="${end%10!=0}">
						<c:forEach var="size" begin="2" end="${(end/10)+1}">
							<a class="nextprevious"
								href="NhaCungCapDB?start=${((size-1)*10)}">${size}</a>
						</c:forEach>
					</c:if>
					<c:if test="${end%10==0}">
